...
{
    return fmt::format("{}: {}", this->user.ToString(), this->content);
}
...
{
#ifdef GUI
    if (BilibiliApi::instance()->RobotIsRunning())
    {
        DanmakuModel::instance()->addMessage(QString::fromStdString(user.GetUname()),
                                             QString::fromStdString(this->content),
                                             user.GetGuardLevelInt());
    }
#endif
    ...
    if (const bool res = DataBase::GetInstance()->AddDanmaku(this->content, this->user); !res)
    {
        LOG_MESSAGE(LogLevel::Error, fmt::format("Failed to add Danmaku to {}", this->ToString()));
    }
}
